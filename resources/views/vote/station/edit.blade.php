...
<body>
    ...
    @section('main')
        ...
        @include('components.breadcrumb', ['page' => 'voteStation', 'pageName' => '投票區', 'title' => '投票'])

        <form class="needs-validation" action="{{route('voteStation.update', $vote)}}" id="voteForm" novalidate method="post" required>
            @csrf
            @method('patch')
            <div class="col-md-12 col-lg-12">
                ...
                <div class="text-end">
                    <button class="btn btn-success" type="submit" id="voteButton">確定投票</button>
        ...
        <script>
            document.getElementById('voteForm').addEventListener('submit', function(event) {
                ...
                const selectedOption = document.querySelector('input[name="voteOptions"]:checked');
                
                if (selectedOption) {
                    const selectedValue = selectedOption.value;
                    const confirmation = confirm(`您確定要選擇以下項目進行投票嗎？\n\n【${selectedOption.closest('.card-body').querySelector('h5').textContent}】\n\n點擊 "確定" 以確認投票，"取消" 取消投票。`);
